...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in kepler.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("kepler.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Kepler features */
    { "kepler",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "kepler",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "kepler",               "gobject",                &gobject,                true,  {} },
    { "kepler",               "voteraw",                &voteraw,                true,  {} },
